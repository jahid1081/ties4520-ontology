(function(){
  ...
  // seriesKey is whats after "Series_": e.g., "The_Bear"
      ...
      .then(function(ttl){
        ...
        for (var i=0;i<quads.length;i++){
          ...
          if (s && s.termType === 'NamedNode'){
            ...
            if (m){ eps.push({ s: +m[1], e: +m[2], id: s.value.split('#').pop() }); }
          }
        }
        ...
        if (!root) return;
        // Clear and build using createElementNS to be XHTML-safe.
        ...
        for (var j=0;j<eps.length;j++){
          ...
          var labelBtn = el('a', { 'class':'btn title-btn',
                                   'href': 'viewer.html?src=individuals.ttl&id=' + encodeURIComponent(ep.id) },
                                   'S'+pad(ep.s)+' · E'+pad(ep.e));
          var openBtn  = el('a', { 'class':'btn',
                                   'href': 'viewer.html?src=individuals.ttl&id=' + encodeURIComponent(ep.id) }, 'Open');
